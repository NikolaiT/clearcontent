	...
	$get_sep = create_function('$key, $array, $sep, $echosep', '$val = clearcontent_next_element_by_key($key, $array); if($val || $echosep) { return "<span class=\"separator\">$sep</span>"; }');
        ...
        if ( isset($show['byline']) && $show['byline'] == true ) {
            printf( '<span class="byline">By <span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s">%3$s</a>%4$s</span></span>',
                    ...
                    $get_sep('byline', $show, $separator, $echosep)
            );
        }
        ...
        if ( 'post' == get_post_type() ) :
			...
			if ( isset($show['posted-in']) && $show['posted-in'] == true ) {
				...
				if ( $categories_list && clearcontent_categorized_blog() ) :
					...
					<span class="cat-links">
						<?php printf( __( 'Posted in %1$s', 'clearcontent' ), $categories_list ); echo $get_sep('posted-in', $show, $separator, $echosep); ?>
					</span>
			...
			if ( isset($show['tags']) && $show['tags'] == true ) {
				...
				<span class="tags-links">
					<?php 
						if ($tags_list) :
							printf( __( 'Tagged %1$s', 'clearcontent' ), $tags_list ); echo $get_sep('tags', $show, $separator, $echosep);
						else :
...
function clearcontent_social_media_icons() {
    ...
    /* Clean recursively */
    $cleaner = function ($item) {return clearcontent_esc_deep($item);};
